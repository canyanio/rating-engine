...
This repository contains the Canyan Rating Engine. 
...
### Requirements
Canyan Rating Engine depends on Canyan Rating API, MongoDB and RabbitMQ. You can easily run them via the provided [docker-compose](docker-compose.yaml) file with:
```
docker-compose up -d
```
...
### Running from source
If you want to run the Engine locally you need to create a Python3 virtual environment with:
```
make venv
```
...
You need to have API, MongoDB and RabbitMQ now running and then you can run the Engine with:
```
make engine
```
